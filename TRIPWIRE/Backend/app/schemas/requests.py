...
class ActionProposalRequest(BaseModel):
    ...
    resource: str = Field(default="", description="Resource target for the action")
    parameters: Any = Field(default_factory=dict, description="Action parameters")

    @field_validator("parameters", mode="before")
    @classmethod
    def validate_parameters(cls, v: Any) -> dict[str, Any]:
        if v is None or v == "[object Object]":
            return {}
        if isinstance(v, str):
            try:
                import json
