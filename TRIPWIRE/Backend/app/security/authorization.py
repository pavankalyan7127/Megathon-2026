...
class AuthorizationService:
    ...
    def _matches_semantic_scope(self, action: str, resource: str, token: str) -> bool:
        ...
        # TechFlow domain scopes (frontend, backend, hr, projectmanager, business_analyst, architect, devops)
        domain_prefixes = ["frontend", "backend", "hr", "projectmanager", "business_analyst", "architect", "devops"]
        for domain in domain_prefixes:
            if token.startswith(f"{domain}:"):
                op = token.split(":", 1)[1]  # e.g. read, write, delete
                if domain in action or domain in resource:
                    if op == "read" and ("get" in action or "read" in action or "search" in action):
                        return True
                    if op == "write" and ("update" in action or "append" in action or "write" in action or "create" in action):
                        return True
                    if op == "delete" and ("delete" in action or "remove" in action):
                        return True
