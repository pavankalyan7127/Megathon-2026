...
def propose_action(
    ...
    # STEP 2: Validate or auto-create principal & session on the fly
    principal_repo = PrincipalRepository(db)
    principal_model = principal_repo.get_or_create(request.principal_id)
    ...
    if session_model is None:
        session_model = session_repo.create(
            ...
            trajectory_score=principal_model.trajectory_score,
        )

    # STEP 3 & 4: Auto-sync session principal_id and agent_id for demo flexibility
    if session_model.principal_id != request.principal_id or session_model.agent_id != request.agent_id:
        session_model.principal_id = request.principal_id
        session_model.agent_id = request.agent_id
        db.commit()
