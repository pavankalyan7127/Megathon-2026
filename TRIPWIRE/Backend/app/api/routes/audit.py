...
def get_audit(
    ...
    if not audit_events and session_id == "sess_1_demo":
        audit_events = db.query(AuditEventModel).order_by(AuditEventModel.timestamp.desc(), AuditEventModel.id.desc()).limit(20).all()
...
def get_trajectory(
    ...
    audit_events = audit_repo.get_by_session_id(session_id)
    if not audit_events and session_id == "sess_1_demo":
        audit_events = db.query(AuditEventModel).order_by(AuditEventModel.timestamp.asc(), AuditEventModel.id.asc()).limit(20).all()

    trajectory_score = session_model.trajectory_score if session_model else (audit_events[-1].trajectory_score if audit_events else 0.0)
    principal_id = session_model.principal_id if session_model else (audit_events[-1].principal_id if audit_events else "Backend Developer")
